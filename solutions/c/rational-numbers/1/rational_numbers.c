...
rational_t exp_rational(rational_t r, int16_t exp){/*raise rational numbers to integer power*/
    int numerator=exp<0?(int)pow((double)r.denominator, (double)abs(exp)) : (int)pow((double)r.numerator, (double)exp);
    int denominator=exp<0?(int)pow((double)r.numerator, (double)abs(exp)) : (int)pow((double)r.denominator, (double)exp);
...
float exp_real(uint16_t base, rational_t r){/*raise real number to rational number*/
    float res=pow((double)base, (double)(1.0/r.denominator));
    return pow(res, r.numerator);
}
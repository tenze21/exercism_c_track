...
saddle_points_t *saddle_points(size_t num_rows, size_t num_cols, uint8_t matrix[][num_cols]){
    saddle_points_t *points = malloc(sizeof(saddle_points_t) + num_rows * num_cols * sizeof(saddle_point_t));
    ...
    for(size_t i=0; i<num_rows; i++){
     ...
     saddle_points_t *tmp= realloc(points, sizeof(saddle_points_t) + points->count * sizeof(saddle_point_t));
     if(tmp){
         points = tmp;
     }
